...
for (t in c(1, 2)) {
  if (t == 1) {
    ...
    analysis <- "recurrence"
    proof_genes_pt <- train_data_recurrence
    
  } else{
    ml_metadata  <-  metadata_ER_POS_SURV
    analysis <- "survival"
    proof_genes_pt <- train_data_survival
  }
...
unpenalized_random_c_indices <- foreach(
  ...
  random_genes <- make.names(random_genes)
...
print(shapiro.test(unpenalized_random_c_indices))
